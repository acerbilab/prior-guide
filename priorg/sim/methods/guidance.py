...
def prior_guide_theta_prior_only(
    ...
    def get_theta(vec):
        return vec[:prior_dim]  # shape (l_p,)
    ...
    def compute_guided_score(x, t):
        ...
        weights = jax.nn.softmax(first_term.squeeze() + second_term)  # (k,)
    ...
    def scan_fn(carry, t0):
        ...
        def langevin_step(state_carry, _):
            ...
            eps = 1e-8
            noise_term = (1 - condition_mask) * jnp.sqrt(2 * langevin_step_size + eps) * noise
        ...
        drift_backward = (1 - condition_mask) * (
            model.sde.drift(t1, x1_after_langevin) - model.sde.diffusion(t1, x1_after_langevin) ** 2 * guided_score
        )
        diffusion_backward = (1 - condition_mask) * (model.sde.diffusion(t1, x1_after_langevin))
        xt0_uncond = (
            x1_after_langevin
            + drift_backward * dt
            + diffusion_backward
