"""
Run Simformer and PriorGuide under Gaussian training priors
"""
...
from priorg.sim.methods.gmm import fit_gmm
...
def evaluate():
    ...
    parser = argparse.ArgumentParser(
        description="Run Simformer and PriorGuide under Gaussian training priors"
    )
    parser.add_argument(
        "--task",
        type=str,
        default="gaussian_linear",
        choices=["gaussian_linear", "gaussian_linear_high", "bav"],
        help="Task name (default: gaussian_linear)",
    )
    parser.add_argument(
        "--prior_type",
        type=str,
    ...
    training_prior_path = exp_path / "data" / "priors" / args.task / f"training.json"
    ...
    p_params = json.load(open(training_prior_path, "r"))

    mu_p = torch.tensor(p_params["mu"])
    sigma_p = torch.tensor(p_params["sigma"])
    ...
    if args.prior_type == "mild" or args.prior_type == "strong":
        mu = torch.tensor(q_params["mu"])
        sigma = torch.tensor(q_params["sigma"])
        q_dist = Independent(
            Normal(loc=mu, scale=sigma),
            reinterpreted_batch_ndims=1,
        )
    elif args.prior_type == "mixture":
        pi = torch.tensor(q_params["pi"])
        mu = torch.tensor(q_params["mu"])
        sigma = torch.tensor(q_params["sigma"])
        # cov = (sigma**2)[..., None] * jnp.eye(sigma.shape[-1])
        q_dist = MixtureSameFamily(
            mixing_distribution=Categorical(probs=pi),
    ...
    key, key_gmm = jax.random.split(key, 2)

    # Fit GMM to the prior
    # _, gmm_params = fit_gmm(
    #     key=key_gmm,
    #     log_target=lambda theta: q_dist.log_prob(theta) - p_dist.log_prob(theta),
    #     q_sampler=lambda key, batch_size: q_dist.sample(key, (batch_size,)),
    #     num_components=20,
    #     dim=mu_p.shape[0],
    #     num_iters=100000,
    #     learning_rate=0.01,
    ...
    _, gmm_params = fit_gmm(
        log_target=lambda theta: q_dist.log_prob(theta) - p_dist.log_prob(theta),
        data_sampler=lambda batch_size: q_dist.sample((batch_size,)),
        num_components=20,
        num_iters=10000,
        learning_rate=1e-3,
        batch_size=100,
    ...
    gmm_log_weights = gmm_params["log_weights"].detach().numpy()
    gmm_means = gmm_params["means"].detach().numpy()
    gmm_stds = jnp.exp(gmm_params["log_stds"].detach().numpy())
    gmm_cov = (gmm_stds**2)[..., None] * jnp.eye(gmm_stds.shape[-1])
    ...
    for obs_seed in observation_seeds:
        ...
        def sample_once_prior(key):
            ...
            return prior_guide_theta_prior_only(
                ...
                x_o=x_o,
                x_T=x_T,
                theta_prior_log_weights=gmm_log_weights,
                theta_prior_mean=gmm_means,
                theta_prior_cov=gmm_cov,
